...
I picked this specific design because its an improvement upon an already existing part in my project. The current issue is that instruction decoding does not account for any padding that my exist in between instructions so a single stepping approach (this project) will be the solution.
